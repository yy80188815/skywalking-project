...
if __name__ == '__main__':
    config.service_name = 'douban.com'
    ...
    def application():
        ...
        spider(66, 67, mysqldb, "douban")
        requests.get("http://127.0.0.1:9080/baidu1")
        from kafka import KafkaProducer
        producer = KafkaProducer(bootstrap_servers=['127.0.0.1:9092'], api_version=(1, 0, 1))
        producer.send('skywalking', b'douban')
        time.sleep(2)
        return jsonify({"status":"okokokok"})


    @app.route("/douban1", methods=["POST", "GET"])
    def application1():
        return jsonify({"status":"okokokok"})
    PORT = 9081
